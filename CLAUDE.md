...
**Never stop a process by pattern.** `pkill`, `killall` and any `-f` pattern are banned here. Deck and the cockpit are both launched as `Electron .`, so on 2026-09-07 a pattern meant for Deck killed Edwin's running cockpit. Several sidecars and Electron apps are always running on this machine, and a Deck from an earlier walk may still be open. Stop only a process you started, by its own pid or its background task id, after `ps -p <pid>` shows the command you expect. A port held by a process you did not start is worked around (`npm run start:lan` moves Deck to the next free port) and reported, never freed.
