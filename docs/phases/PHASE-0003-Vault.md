...
related: ["[[REFERENCE-SURFACE-ARCHITECTURE-OPTIONS]]", "[[REFERENCE-COCKPIT-ADOPTION]]", "[[PHASE-0001-Deck]]", "[[PHASE-0002-Glass]]"]
...
**Depends on [[PHASE-0001-Deck]], not on [[PHASE-0002-Glass]].** The seam this phase needs is the view provider the Deck phase builds into the renderer; this phase adds the provider that reads a vault's `.base` files. Vault support is Deck's, not the cockpit's, so the question of what Deck reads itself and what it still takes from the sidecar's index is settled when this phase opens. Nothing here requires the field. The stated order still puts Vault third, which is Edwin's decision on 2026-09-06, not a technical constraint; grooming can bring it forward if a vault becomes the more valuable demonstration.
