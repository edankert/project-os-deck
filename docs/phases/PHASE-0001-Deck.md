...
- **Views come from the sidecar.** Deck asks which views a workspace has and hard-codes none. The sidecar gains that answer here, returning the project-os nav modes for a repo, so that a vault's bases can be views later without a retrofit.
...
- **Obsidian vaults**: the workspace profile, bases as views, and `.canvas` files as boards. That is [[PHASE-0003-Vault]]. Only the seam is built here, and the seam is the sidecar answering which views a workspace has.
...
- [ ] Every view in Deck's switcher came from the sidecar's answer, and no list of view names is written in the renderer.
