...
**The second review, the same day, requested changes again, and its findings are two more issues.** A reduced-motion lift from a turned field could leave cards under a pane in the slot model, a pull beside a pane on a narrow field could vanish while the label counted it, an unnamed display was numbered by its place in Electron's list, and the reduced-motion landing did not highlight its target ([[ISS-0064-A-Reduced-Motion-Lift-And-A-Pull-Beside-A-Pane-Still-Misplace-Cards]]). Five checks could not fail for the defect they were named for, and four notes were stale ([[ISS-0065-Five-Checks-Still-Cannot-Fail-And-Four-Notes-Are-Stale]]). Both are fixed. What a person notices: a note just pulled stays in view beside a pane, and an owed note is counted off the field instead; a display macOS calls " (1)" reads "display 1"; under reduced motion a throw leaves the strip up for a moment with the target's name highlighted. Edwin stopped the review loop there, and on 2026-09-11 marked the three features `done` himself, without an approved review.
