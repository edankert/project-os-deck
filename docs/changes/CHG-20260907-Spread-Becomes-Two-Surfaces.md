...
- **Finished work is recognised from either signal the sidecar sends.** The features view marks one group `suppressed`. The issues view does not use that flag at all: it repeats each severity band for the finished issues and suffixes the key with `:done`. Deck reads both, so Your Trainer's Issues view opens on the 34 that need triage with 311 finished ones folded into nine rows.
