...
- [ ] Write the recogniser over pointer samples and the window book's bounds, with the thresholds as inputs; write the chosen numbers here.
- [ ] Draw the target strip from the windows in the drag's direction, with the tablet as a target when a served page is subscribed.
- [ ] Wire a release to the existing focus, put-on-desk and open-panel actions; place a new window with the existing placement function.
- [ ] Add the flight and the reduced-motion cut.
- [ ] Add send to on the navigator row and the pane header.
- [ ] Extend the smoke run with two windows: a throw to a reader, a throw to a desk, a throw to an empty display, and the keyboard route.
