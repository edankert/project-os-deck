...
**The comparison with the sidecar is real rather than a formality.** `desktop/fixtures/sidecar-types.json` is recorded from the cockpit's own `Index`, by `tools/scripts/record-sidecar-fixture.py`, which imports it. Deck agrees with the sidecar about every note in this repository, path by path, and about Your Trainer's, to within the differences the fixture names one by one: a list-valued `type:` Deck must not reproduce ([[project-os-cockpit#ISS-0279]]), and files whose frontmatter PyYAML refuses outright, whose notes therefore vanish from the cockpit's own views. **The counts are not written here.** They were, twice, and both were stale within a day — a count of a live corpus is a measurement of a moving thing ([[ISS-0052-A-Fourth-Round-Of-Numbers-That-Do-Not-Reproduce]]). `tools/scripts/check-counts-live.py` prints today's.
