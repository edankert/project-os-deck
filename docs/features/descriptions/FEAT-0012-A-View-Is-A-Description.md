...
- **An evaluator for the seeded subset** over Deck's index: filters with `and`, `or`, `not`, the six comparison operators, the functions the vault's own base files actually use — `tools/scripts/check-bases-live.mjs` prints how many there are today, and the count is not written here because it moved twice ([[ISS-0052-A-Fourth-Round-Of-Numbers-That-Do-Not-Reproduce]]) — sort, `groupBy`, and formulas as far as the seed needs them.
