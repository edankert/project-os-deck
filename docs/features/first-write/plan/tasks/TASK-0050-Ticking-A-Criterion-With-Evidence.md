...
- [ ] Read `data-raw` from the rendered HTML in the reader and attach a control to each checkbox that has one.
- [ ] Detect the no-attributes case and draw the explanation instead of the controls.
- [ ] Collect evidence in the control, then post through the write channel with the actor and the modification time.
- [ ] Word the three refusals — nothing matched, matched twice, changed underneath — and test each against a fake sidecar.
- [ ] Re-read and redraw the note after a successful tick.
- [ ] Extend [[TST-0033-The-Write-Channel-Exists-In-The-Shell-And-Not-When-Served]] with the tick cases.
