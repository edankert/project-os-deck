...
This adds one, in Deck: a field of nodes coloured by status band, clustered by phase, entered by flying rather than by scrolling. It is **Deck's Glass view** ([[PHASE-0002-Glass]]), not a replacement for anything.
...
- The field as a Deck view with its own address (the address grammar is [[PHASE-0001-Deck]]'s), reachable from Deck's view switcher and from any note ("show this in the field")
...
- Fly, zoom, and land: landing opens the note in Deck's reader, the pane Spread already opens notes in, rather than in a bespoke panel
