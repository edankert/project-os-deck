...
- 2026-09-08 — Edwin asked for a review of the architecture before Glass is built, against four additions: views as descriptions, generated editors, writes including checkboxes in a note's body, and flows. The review is [[REFERENCE-ARCHITECTURE-REVIEW-BEFORE-GLASS]]. It finds the seams of Part 4 sound and the view record too small, and recommends reopening [[PHASE-0001-Deck]] for a view description, a write path through the shell, an open address grammar and flows as a concept. Part 4's layers table and the "reads only" rule of Part 8 item 1 are the sections it makes historical.
- 2026-09-06 — Part 8 item 2's constraint that the sidecar answers which views a workspace has is revised by Edwin: the list of views is Deck's concern and the sidecar is not asked to change. Deck's renderer takes its views from a provider chosen by workspace kind; the project-os provider is built in and matches the cockpit's navigator, and the Vault phase adds one that reads `.base` files. The retrofit the item warned about is avoided the same way, inside Deck. Recorded in [[PHASE-0001-Deck]], [[PHASE-0003-Vault]] and `CLAUDE.md`.
